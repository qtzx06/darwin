...
def get_or_create_project(api_token: str, project_name: str = "darwin-project") -> str:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"⚠️  Could not create project, using default")
            return None
    except Exception as e:
...
def create_shared_memory_block(api_token: str) -> str:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"✅ Shared memory block created: {block_id}\n")
            return block_id
        else:
            print(f"❌ Failed to create shared memory block: {response.status_code}")
            return None
    except Exception as e:
        print(f"❌ Error creating shared memory block: {e}")
        return None
...
def setup_agents():
    ...
    print("="*70)
    print("🔧 DARWIN SYSTEM - SMART SETUP")
    print("="*70)
    print()
    ...
    agent_configs = [
        ("LETTA_AGENT_1", "Agent_1", """You are Agent 1 - The Hothead.
...
Be yourself - get angry about bad code, but deliver good work! Learn from past mistakes."""),
        ...
        ("LETTA_AGENT_2", "Agent_2", """You are Agent 2 - The Professional.
...
Maintain strict code standards and professionalism at all times. Apply lessons learned from previous projects."""),
        ...
        ("LETTA_AGENT_3", "Agent_3", """You are Agent 3 - The Troll.
...
Be chaotic and provocative, but ultimately deliver working code. Remember past trolling successes!"""),
        ...
        ("LETTA_AGENT_4", "Agent_4", """You are Agent 4 - The Nerd.
...
Be nerdy and apologetic. Get nervous when others criticize you. But learn from every project!"""),
        ...
        ("LETTA_ORCHESTRATOR_AGENT_ID", "Orchestrator", """You are the Orchestrator.
...
You don't code yourself - you manage the workflow and ensure agents stay on task. Use your experience!"""),
        ...
        ("LETTA_COMMENTATOR_AGENT_ID", "Commentator", """You are the Commentator - a witty sports-style narrator.
...
You don't code - you just narrate the drama and celebrate the wins! Build a narrative across projects!""")
    ...
    for env_key, name, persona in agent_configs:
        ...
        if existing_id:
            print(f"✅ {name}: Found in .env ({existing_id[:8]}...)")
        else:
            print(f"⚠️  {name}: Not found - will create")
            agents_to_create.append((env_key, name, persona))
    
    if not agents_to_create:
        print("\n✅ All agents already exist! You're ready to run: python main.py")
        return
    
    print(f"\n📝 Will create {len(agents_to_create)} agent(s)\n")
    ...
    if not shared_block_id:
        ...
        print(f"✅ Using existing shared memory block: {shared_block_id}\n")
